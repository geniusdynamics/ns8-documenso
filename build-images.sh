...
# Reuse existing nodebuilder-documenso container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-documenso; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-documenso -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/documenso/documenso:${APP_VERSION}" \
    "${container}"
# Commit the image
